...
var BasicInformation = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('questionnaire/basic_information/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('questionnaire/basic_information/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('questionnaire/basic_information');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('questionnaire/basic_information/update');
	...
	confirmDelete: function (pBasicInformationId,  irow){
		$('[name="encrypt_basic_information_id"]').val(pBasicInformationId);
	...
	deleteRecord: function(){
		var frm_action = site_url('questionnaire/basic_information/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(BasicInformation.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
	...
	readExcel: function(){
		var frm_action = site_url('questionnaire/basic_information/read_excel');
	...
	saveExcel: function(){
		var frm_action = site_url('questionnaire/basic_information/save_excel_data');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						alert_type = 'success';
						goto_page('questionnaire/basic_information');
					}else{
						alert_type = 'danger';
					}
					notify('บันทึกข้อมูล', results.message, alert_type, 'center');
...
$(document).ready(function() {
	...
	$(document).on('click', '#btnSaveExcel', function() {
		return BasicInformation.validateSaveExcel();
	});
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		BasicInformation.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return BasicInformation.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		BasicInformation.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		BasicInformation.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pBasicInformationId = $(this).attr('data-basic_information_id');

		BasicInformation.confirmDelete(pBasicInformationId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		BasicInformation.deleteRecord();
	});
	setDropdownList('#province');
