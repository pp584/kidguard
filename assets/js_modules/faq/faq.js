...
var Faq = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('faq/faq/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('faq/faq/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('faq/faq');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('faq/faq/update');
	...
	confirmDelete: function (pCmsFaqId,  irow){
		$('[name="encrypt_cms_faq_id"]').val(pCmsFaqId);
	...
	deleteRecord: function(){
		var frm_action = site_url('faq/faq/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Faq.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
	...
	readExcel: function(){
		var frm_action = site_url('faq/faq/read_excel');
	...
	saveExcel: function(){
		var frm_action = site_url('faq/faq/save_excel_data');
...
$(document).ready(function() {
	...
	if(typeof param_current_page != 'undefined'){
		Faq.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Faq.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pCmsFaqId = $(this).attr('data-cms_faq_id');

		Faq.confirmDelete(pCmsFaqId,  row_num);
	});//click
