...
var assessment_create = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('assessment/assessment_create/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('assessment/assessment_create/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('assessment/assessment1/add');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('assessment/assessment_create/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('assessment/assessment_create/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(assessment_create.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		assessment_create.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return assessment_create.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		assessment_create.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		assessment_create.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		assessment_create.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		assessment_create.deleteRecord();
	});
