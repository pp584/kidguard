...
var assessment4 = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('assessment/assessment4/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('assessment/assessment4/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('assessment/AssessmentPreview');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('assessment/assessment4/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('assessment/assessment4/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(assessment4.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		assessment4.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return assessment4.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		assessment4.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		assessment4.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		assessment4.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		assessment4.deleteRecord();
	});
