...
var assessment3 = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('assessment/assessment3/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('assessment/assessment3/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('assessment/assessment4/add');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('assessment/assessment3/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('assessment/assessment3/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(assessment3.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		assessment3.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return assessment3.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		assessment3.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		assessment3.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		assessment3.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		assessment3.deleteRecord();
	});
