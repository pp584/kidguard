...
var CaSymptoms = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ca_symptoms/ca_symptoms/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ca_symptoms/ca_symptoms/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ca_symptoms/ca_symptoms/update');
	...
	confirmDelete: function (pCaSymptomsId,  irow){
		$('[name="encrypt_ca_symptoms_id"]').val(pCaSymptomsId);
	...
	deleteRecord: function(){
		var frm_action = site_url('ca_symptoms/ca_symptoms/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(CaSymptoms.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		CaSymptoms.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return CaSymptoms.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		CaSymptoms.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		CaSymptoms.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pCaSymptomsId = $(this).attr('data-ca_symptoms_id');

		CaSymptoms.confirmDelete(pCaSymptomsId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		CaSymptoms.deleteRecord();
	});
