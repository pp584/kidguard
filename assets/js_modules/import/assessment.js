...
var Assessment = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('import/assessment/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('import/assessment/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('import/assessment');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('import/assessment/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('import/assessment/del');
	...
	readExcel: function(){
		var frm_action = site_url('import/assessment/read_excel');
	...
	saveExcel: function(){
		var frm_action = site_url('import/assessment/save_excel_data');
	...
	validateSaveExcel: function(){
		var message = '';
			...
			var count_province_id = $('#frmImportList input[name^="province_id["]').filter(function(){
				return !$(this).val();
			}).length;
			
			if(count_province_id > 0){
				message += "- ไม่พบรหัสอ้างอิง จังหวัด " + count_province_id + " รายการ \n";
			}
...
$(document).ready(function() {
	...
	$(document).on('click', '#btnSaveExcel', function() {
		return Assessment.validateSaveExcel();
	});
