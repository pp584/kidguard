...
var Room = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('demo/room/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('demo/room/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('demo/room/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('demo/room/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Room.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	if(typeof param_current_page != 'undefined'){
		Room.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Room.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Room.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Room.deleteRecord();
	});
	setDropdownList('#ref_teacher_id');
