...
var DeliveryInformation = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('services/delivery_information/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('services/delivery_information/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('services/delivery_information');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('services/delivery_information/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('services/delivery_information/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(DeliveryInformation.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		DeliveryInformation.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return DeliveryInformation.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		DeliveryInformation.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		DeliveryInformation.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		DeliveryInformation.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		DeliveryInformation.deleteRecord();
	});
