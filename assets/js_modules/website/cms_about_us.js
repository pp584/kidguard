...
var CmsAboutUs = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('website/cms_about_us/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('website/cms_about_us/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('website/cms_about_us/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('website/cms_about_us/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(CmsAboutUs.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		CmsAboutUs.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return CmsAboutUs.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		CmsAboutUs.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		CmsAboutUs.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		CmsAboutUs.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		CmsAboutUs.deleteRecord();
	});
