...
var CmsTeams = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('teams/cms_teams/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('teams/cms_teams/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('teams/cms_teams/update');
	...
	confirmDelete: function (pCmsTeamId,  irow){
		$('[name="encrypt_cms_team_id"]').val(pCmsTeamId);
	...
	deleteRecord: function(){
		var frm_action = site_url('teams/cms_teams/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(CmsTeams.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		CmsTeams.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return CmsTeams.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		CmsTeams.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		CmsTeams.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pCmsTeamId = $(this).attr('data-cms_team_id');

		CmsTeams.confirmDelete(pCmsTeamId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		CmsTeams.deleteRecord();
	});
