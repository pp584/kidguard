...
var CmsCaResuscitation = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ca_resucitation/cms_ca_resuscitation/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ca_resucitation/cms_ca_resuscitation/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ca_resucitation/cms_ca_resuscitation/update');
	...
	confirmDelete: function (pCaResuscitationId,  irow){
		$('[name="encrypt_ca_resuscitation_id"]').val(pCaResuscitationId);
	...
	deleteRecord: function(){
		var frm_action = site_url('ca_resucitation/cms_ca_resuscitation/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(CmsCaResuscitation.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		CmsCaResuscitation.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return CmsCaResuscitation.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		CmsCaResuscitation.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		CmsCaResuscitation.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pCaResuscitationId = $(this).attr('data-ca_resuscitation_id');

		CmsCaResuscitation.confirmDelete(pCaResuscitationId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		CmsCaResuscitation.deleteRecord();
	});
