...
var AssessmentForm = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('report/assessment_form/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('report/assessment_form/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('report/assessment_form');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('report/assessment_form/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('report/assessment_form/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(AssessmentForm.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		AssessmentForm.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return AssessmentForm.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		AssessmentForm.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		AssessmentForm.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		AssessmentForm.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		AssessmentForm.deleteRecord();
	});
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('report/assessment_form/' + $(this).attr('onclick-action'));
