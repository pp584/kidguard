...
var Research = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('research/research/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('research/research/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('research/research/update');
	...
	confirmDelete: function (pCmsResearchsId,  irow){
		$('[name="encrypt_cms_researchs_id"]').val(pCmsResearchsId);
	...
	deleteRecord: function(){
		var frm_action = site_url('research/research/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Research.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Research.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Research.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Research.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Research.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pCmsResearchsId = $(this).attr('data-cms_researchs_id');

		Research.confirmDelete(pCmsResearchsId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Research.deleteRecord();
	});
