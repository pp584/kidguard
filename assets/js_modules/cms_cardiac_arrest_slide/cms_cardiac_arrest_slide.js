...
var CmsCardiacArrestSlide = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('cms_cardiac_arrest_slide/cms_cardiac_arrest_slide/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('cms_cardiac_arrest_slide/cms_cardiac_arrest_slide/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('cms_cardiac_arrest_slide/cms_cardiac_arrest_slide/update');
	...
	confirmDelete: function (pCmsCardiacArrestId,  irow){
		$('[name="encrypt_cms_cardiac_arrest_id"]').val(pCmsCardiacArrestId);
	...
	deleteRecord: function(){
		var frm_action = site_url('cms_cardiac_arrest_slide/cms_cardiac_arrest_slide/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(CmsCardiacArrestSlide.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		CmsCardiacArrestSlide.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return CmsCardiacArrestSlide.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		CmsCardiacArrestSlide.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		CmsCardiacArrestSlide.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pCmsCardiacArrestId = $(this).attr('data-cms_cardiac_arrest_id');

		CmsCardiacArrestSlide.confirmDelete(pCmsCardiacArrestId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		CmsCardiacArrestSlide.deleteRecord();
	});
