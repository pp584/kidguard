...
function loading_after(obj) {
	$(obj).after(
		'<span id="loading_after">&nbsp;&nbsp;<i class="fas fa-sync fa-spin"></i> Loading..</span>'
	);
}
			...
			if (parseInt(extall.indexOf(ext)) >= 0) {
				if (!$(elem_display + "_iframe").attr("id")) {
					var iframe_preview =
						...
						'_iframe" frameborder="0" scrolling="no" width="400" height="600" style="border : 1px solid #ccc"></iframe>';
...
function ajaxErrorMessage(jqXHR, exception, elem) {
	...
	console.error("[ajaxErrorMessage]: response ==> ", {
		status: jqXHR.status,
...
function getGraphBarColor() {
	const colors = ["#300240", "#7238C9", "#D806DD", "#EFCD4A","#0B7EF3","#FF6FB1"];
	return colors;
}
