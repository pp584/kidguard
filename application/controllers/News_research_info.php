 ...
 */
class News_research_info extends CRUD_Controller
{
	private $per_page;
	private $another_js;
	private $another_css;
	...
	private function render_view($path)
	{
		...
		$this->parser->parse('template/front-end/news_research_info', $this->data);
	}


	public function cms_posts()
	{
		$cms_about_us = $this->modeldb->get_list("cms_researchs");
		return $cms_about_us;
	}
	...
	public function preview($encrypt_id = "")
	{
		...
		if ($id == "") {
			...
			$cms_about_us = $this->modeldb->get_list("cms_researchs", "cms_researchs_id=$id");
			$this->data['results'] = $cms_about_us;

			$this->parser->parse('template/front-end/news_research_info_view', $this->data);
		}
	}
	...
	public function update_page_view($id){
		...
		$this->db->query("UPDATE cms_researchs set page_views_total = page_views_total + 1 WHERE cms_researchs_id=$id ");
