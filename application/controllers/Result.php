 ...
 */
class Result extends CRUD_Controller
{
	private $per_page;
	private $another_js;
	private $another_css;
	...
	private function render_view($path)
	{
		...
		$this->data['results']	=  $this->cms_posts();
		$this->parser->parse('template/front-end/result_view', $this->data);
	}


	public function cms_posts()
	{
		$cms_about_us = $this->modeldb->get_list("cms_ca_resuscitation");
		return $cms_about_us;
	}
	...
	public function preview($encrypt_id = "")
	{
		...
		if ($id == "") {
			...
			$cms_about_us = $this->modeldb->get_list("cms_ca_resuscitation", "ca_resuscitation_id=$id");
	...
	public function update_page_view($id){
		...
		$this->db->query("UPDATE cms_ca_resuscitation set page_views_total = page_views_total + 1 WHERE ca_resuscitation_id =$id ");
	 ...
	 * SET array data list
	 */
	private function setDataListFormat_cms_ca_resuscitation($lists_data, $start_row = 0)
	{
		$data = $lists_data;
		...
		for ($i = 0; $i < $count; $i++) {
			...
			$pk1 = $data[$i]['cms_ca_resuscitation'];
			$data[$i]['url_encrypt_id'] = urlencode(encrypt($pk1));

			if ($pk1 != '') {
				$pk1 = ci_encrypt($pk1);
			}
			$data[$i]['encrypt_cms_ca_resuscitation'] = $pk1;
