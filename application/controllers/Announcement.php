 ...
 */
class Announcement extends CRUD_Controller
{
	private $per_page;
	private $another_js;
	private $another_css;
	...
	private function render_view($path)
	{
		...
		$this->parser->parse('template/front-end/newspage_view', $this->data);
	}


	public function cms_posts()
	{
		$cms_about_us = $this->modeldb->get_list("cms_posts", "category_id=1 and status = 'published'");
		return $cms_about_us;
	}
	...
	public function preview($encrypt_id = "")
	{
		...
		if ($id == "") {
			...
			$cms_about_us = $this->modeldb->get_list("cms_posts", "id=$id");
			$this->data['results'] = $cms_about_us;

			$this->update_page_view($id);


			$this->parser->parse('template/front-end/newspage_view_detail', $this->data);
		}
	}
	...
	public function update_page_view($id)
	{
		...
		$this->db->query("UPDATE cms_posts set page_views_total = page_views_total + 1 WHERE id=$id ");
