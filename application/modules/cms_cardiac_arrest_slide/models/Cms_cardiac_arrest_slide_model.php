...
/**
 * Cms_cardiac_arrest_slide_model Class
 * @date 2022-04-18
 */
class Cms_cardiac_arrest_slide_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'cms_cardiac_arrest_slide';
	...
	{
		$cms_cardiac_arrest_id = checkEncryptData($data['cms_cardiac_arrest_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.cms_cardiac_arrest_id = $cms_cardiac_arrest_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.cms_cardiac_arrest_id = $id");
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'cms_cardiac_arrest_id'){
				if(!is_numeric($value)){
					$value = 0;
				}
				$value = $value + 0;
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$cms_cardiac_arrest_id = checkEncryptData($post['encrypt_cms_cardiac_arrest_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.cms_cardiac_arrest_id = $cms_cardiac_arrest_id");
