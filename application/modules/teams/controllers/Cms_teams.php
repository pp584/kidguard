...
/**
 * [ Controller File name : Cms_teams.php ]
 */
class Cms_teams extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('teams/Cms_teams_model', 'Cms_teams');
		$this->Cms_teams->session_name = 'teams_cms_teams';

		$this->load->model('FileUpload_model', 'FileUpload');
		$this->data['page_url'] = site_url('teams/cms_teams');
		...
		$this->upload_store_path = './assets/uploads/cms_teams/';
		...
		$js_url = 'assets/js_modules/teams/cms_teams.js?ft='. filemtime('assets/js_modules/teams/cms_teams.js');
	...
	public function index()
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'จัดการข้อมูลทีมงานวิจัย', 'class' => 'active', 'url' => '#'),
		);
	...
	public function list_all() {
		$this->session->unset_userdata($this->Cms_teams->session_name . '_search_field');
		$this->session->unset_userdata($this->Cms_teams->session_name . '_value');
	...
	public function search()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$arr = array($this->Cms_teams->session_name . '_search_field' => $search_field, $this->Cms_teams->session_name . '_value' => $value );
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->Cms_teams->session_name . '_search_field');
			$value = $this->session->userdata($this->Cms_teams->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->Cms_teams->order_field = $field;
			$this->Cms_teams->order_sort = $sort;
		}
		$results = $this->Cms_teams->read($start_row, $per_page);
		$total_row = $results['total_row'];
		$search_row = $results['search_row'];
		$list_data = $this->setDataListFormat($results['list_data'], $start_row);


		$page_url = site_url('teams/cms_teams');
		...
		$this->render_view('teams/cms_teams/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'จัดการข้อมูลทีมงานวิจัย', 'url' => site_url('teams/cms_teams')),
		...
		if ($id == "") {
			...
			$results = $this->Cms_teams->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('teams/cms_teams/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		$this->render_view('teams/cms_teams/add_view');
	}
	...
	public function formValidate()
	{
		...
		if($check_file == TRUE){
			if(empty($_FILES['photo']['name'])){
				$frm->set_rules('photo', 'photo', 'trim|required');
			}
		}
		$frm->set_rules('name', 'ชื่อ-นามสกุล', 'trim|required');
		$frm->set_rules('company_name', 'หน่วยงาน', 'trim|required');
		$frm->set_rules('tel', 'เบอร์โทร', 'trim|required');
		$frm->set_rules('level', 'ลำดับ', 'trim|required|is_natural');
	...
	public function formValidateWithFile()
	{
		...
		if(!empty($_FILES['photo']['name'])){
			...
			$frm->set_rules('photo', 'photo', 'callback_file_check');
			if ($frm->run() == FALSE) {
				$message .= form_error('photo');
			}
		}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				...
				$this->render_view('teams/cms_teams/edit_view');
			}
		}
	}
	...
	{
		$error = '';
		$cms_team_id = ci_decrypt($data['encrypt_cms_team_id']);
		if($cms_team_id==''){
			$error .= '- รหัส cms_team_id';
		}
		return $error;
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			if($upload_error == 0){
				$result = $this->Cms_teams->update($post);
				if($result == false){
					$message = $this->Cms_teams->error_message;
					$ok = FALSE;
				}else{
					$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Cms_teams->error_message;
					$ok = TRUE;
				}
			}else{
				$ok = FALSE;
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->Cms_teams->delete($post);
			if($result == false){
				$message = $this->Cms_teams->error_message;
				$ok = FALSE;
			}else{
	...
	private function setDataListFormat($lists_data, $start_row=0)
	{
		...
		for($i=0;$i<$count;$i++){
			...
			$pk1 = $data[$i]['cms_team_id'];
			$data[$i]['url_encrypt_id'] = urlencode(encrypt($pk1));

			if($pk1 != ''){
				$pk1 = ci_encrypt($pk1);
			}
			$data[$i]['encrypt_cms_team_id'] = $pk1;
	...
	private function setPreviewFormat($row_data)
	{
		...
		$pk1 = $data['cms_team_id'];
		$this->data['recode_url_encrypt_id'] = urlencode(encrypt($pk1));

		if($pk1 != ''){
			$pk1 = ci_encrypt($pk1);
		}
		$this->data['encrypt_cms_team_id'] = $pk1;

		$this->data['record_cms_team_id'] = $data['cms_team_id'];
