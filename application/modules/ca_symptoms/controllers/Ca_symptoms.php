...
/**
 * [ Controller File name : Ca_symptoms.php ]
 */
class Ca_symptoms extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('ca_symptoms/Ca_symptoms_model', 'Ca_symptoms');
		$this->Ca_symptoms->session_name = 'ca_symptoms_ca_symptoms';

		$this->load->model('FileUpload_model', 'FileUpload');
		$this->data['page_url'] = site_url('ca_symptoms/ca_symptoms');
		...
		$this->upload_store_path = './assets/uploads/ca_symptoms/';
		...
		$js_url = 'assets/js_modules/ca_symptoms/ca_symptoms.js?ft='. filemtime('assets/js_modules/ca_symptoms/ca_symptoms.js');
	...
	public function index()
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'จัดการข้อมูลที่ควรรู้', 'class' => 'active', 'url' => '#'),
		);
	...
	public function list_all() {
		$this->session->unset_userdata($this->Ca_symptoms->session_name . '_search_field');
		$this->session->unset_userdata($this->Ca_symptoms->session_name . '_value');
	...
	public function search()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$arr = array($this->Ca_symptoms->session_name . '_search_field' => $search_field, $this->Ca_symptoms->session_name . '_value' => $value );
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->Ca_symptoms->session_name . '_search_field');
			$value = $this->session->userdata($this->Ca_symptoms->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->Ca_symptoms->order_field = $field;
			$this->Ca_symptoms->order_sort = $sort;
		}
		$results = $this->Ca_symptoms->read($start_row, $per_page);
		$total_row = $results['total_row'];
		$search_row = $results['search_row'];
		$list_data = $this->setDataListFormat($results['list_data'], $start_row);


		$page_url = site_url('ca_symptoms/ca_symptoms');
		...
		$this->render_view('ca_symptoms/ca_symptoms/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'จัดการข้อมูลที่ควรรู้', 'url' => site_url('ca_symptoms/ca_symptoms')),
		...
		if ($id == "") {
			...
			$results = $this->Ca_symptoms->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('ca_symptoms/ca_symptoms/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		$this->render_view('ca_symptoms/ca_symptoms/add_view');
	}
	...
	public function formValidate()
	{
		...
		$frm->set_rules('ca_symptoms_name', 'ca_symptoms_name', 'trim|required');
		$frm->set_rules('ca_symptoms_detail', 'ca_symptoms_detail', 'trim|required');
		...
		if ($frm->run() == FALSE) {
			$message  = '';
			$message .= form_error('ca_symptoms_name');
			$message .= form_error('ca_symptoms_detail');
			return $message;
		}
	}
	...
	public function save()
	{
		...
		if ($message != '') {
			...
			if($upload_error == 0){
				$success = TRUE;
				$id = $this->Ca_symptoms->create($post);
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				...
				$this->render_view('ca_symptoms/ca_symptoms/edit_view');
			}
		}
	}
	...
	{
		$error = '';
		$ca_symptoms_id = ci_decrypt($data['encrypt_ca_symptoms_id']);
		if($ca_symptoms_id==''){
			$error .= '- รหัส ca_symptoms_id';
		}
		return $error;
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			if($upload_error == 0){
				$result = $this->Ca_symptoms->update($post);
				if($result == false){
					$message = $this->Ca_symptoms->error_message;
					$ok = FALSE;
				}else{
					$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Ca_symptoms->error_message;
					$ok = TRUE;
				}
			}else{
				$ok = FALSE;
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->Ca_symptoms->delete($post);
			if($result == false){
				$message = $this->Ca_symptoms->error_message;
				$ok = FALSE;
			}else{
	...
	private function setDataListFormat($lists_data, $start_row=0)
	{
		...
		for($i=0;$i<$count;$i++){
			...
			$pk1 = $data[$i]['ca_symptoms_id'];
			$data[$i]['url_encrypt_id'] = urlencode(encrypt($pk1));

			if($pk1 != ''){
				$pk1 = ci_encrypt($pk1);
			}
			$data[$i]['encrypt_ca_symptoms_id'] = $pk1;
	...
	private function setPreviewFormat($row_data)
	{
		...
		$pk1 = $data['ca_symptoms_id'];
		$this->data['recode_url_encrypt_id'] = urlencode(encrypt($pk1));

		if($pk1 != ''){
			$pk1 = ci_encrypt($pk1);
		}
		$this->data['encrypt_ca_symptoms_id'] = $pk1;

		$this->data['record_ca_symptoms_id'] = $data['ca_symptoms_id'];
		$this->data['record_ca_symptoms_name'] = $data['ca_symptoms_name'];
		$this->data['record_ca_symptoms_detail'] = $data['ca_symptoms_detail'];
