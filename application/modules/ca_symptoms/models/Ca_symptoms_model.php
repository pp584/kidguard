...
/**
 * Ca_symptoms_model Class
 * @date 2022-04-18
 */
class Ca_symptoms_model extends MY_Model
{
	...
	{
		$ca_symptoms_id = checkEncryptData($data['ca_symptoms_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.ca_symptoms_id = $ca_symptoms_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.ca_symptoms_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				'ca_symptoms_name' => $post['ca_symptoms_name']
				,'ca_symptoms_detail' => $post['ca_symptoms_detail']
				,'image' => $post['image']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'ca_symptoms_id'){
				if(!is_numeric($value)){
					$value = 0;
				}
				$value = $value + 0;
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$ca_symptoms_id = checkEncryptData($post['encrypt_ca_symptoms_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.ca_symptoms_id = $ca_symptoms_id");
