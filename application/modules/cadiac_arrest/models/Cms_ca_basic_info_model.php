...
/**
 * Cms_ca_basic_info_model Class
 * @date 2022-04-10
 */
class Cms_ca_basic_info_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'cms_ca_basic_info';
	...
	{
		$ca_basic_info = checkEncryptData($data['ca_basic_info']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.ca_basic_info = $ca_basic_info");
		return $this->count_record();
	}



	public function load_home($id)
	{
		$query   =$this->db->select("$this->my_table.*");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.ca_basic_info = $id");
	...
	public function create($post)
	{
		...
		$data = array(
			'subject' => $post['subject'], 'detail' => $post['detail'], 'images' => $post['images'], 'create_by_userid' => $post['create_by_userid'], 'files' => $post['files']
		);
		$this->set_table_name($this->my_table);
	...
	{
		$data = array(
			'subject' => $post['subject'], 'detail' => $post['detail'], 'create_by_userid' => $post['create_by_userid']
		);
		...
		if (!empty($data)) {
			$ca_basic_info = checkEncryptData($post['encrypt_ca_basic_info']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.ca_basic_info = $ca_basic_info");
