...
class Cms_ca_basic_info extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->Cms_ca_basic_info->session_name = 'cadiac_arrest_cms_ca_basic_info';

		$this->load->model('FileUpload_model', 'FileUpload');
		$this->data['page_url'] = site_url('cadiac_arrest/cms_ca_basic_info');
		...
		$js_url = 'assets/js_modules/cadiac_arrest/cms_ca_basic_info.js?ft='. filemtime('assets/js_modules/cadiac_arrest/cms_ca_basic_info.js');
	...
	public function index()
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'Cms_ca_basic_info', 'class' => 'active', 'url' => '#'),
		);
	...
	public function search()
	{
		...
		$page_url = site_url('cadiac_arrest/cms_ca_basic_info');
		...
		$this->render_view('cadiac_arrest/cms_ca_basic_info/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'Cms_ca_basic_info', 'url' => site_url('cadiac_arrest/cms_ca_basic_info')),
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('cadiac_arrest/cms_ca_basic_info/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		$this->setFormAddData();
		$this->render_view('cadiac_arrest/cms_ca_basic_info/add_view');
	}
	...
	public function formValidate()
	{
		...
		//file upload
		$check_file = FALSE;
		if($this->input->post('images_label') == ''){
			$check_file = TRUE;
		}
		if($check_file == TRUE){
			if(empty($_FILES['images']['name'])){
				$frm->set_rules('images', 'รูปภาพ', 'trim|required');
			}
		}
		//file upload
		$check_file = FALSE;
		...
		if($check_file == TRUE){
			if(empty($_FILES['files']['name'])){
				$frm->set_rules('files', 'ไฟล์แนบ', 'trim|required');
			}
		}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				...
				$this->data['source_create_by_userid'] = $this->session->userdata('user_id');

				$this->setPreviewFormat($results);

				$this->data['record_create_date'] = setThaiDate($results['create_date']);

				$this->render_view('cadiac_arrest/cms_ca_basic_info/edit_view');
			}
		}
	}
