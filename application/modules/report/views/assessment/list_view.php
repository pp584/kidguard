...
<div class="card">
	<div class="card-header bg-primary">
		<h3 class="card-title"><i class="fa fa-list-alt"></i> ออกรายงาน</h3>
	</div>
	<div class="card-body">
		<div class="row">
			...
			<table class="table table-bordered table-striped table-hover">
				<thead class="info">
					<tr bgcolor="#dddddd">
						...
						<th>การรับรู้แหล่งซื้อสารเสพติด</th>
						<th>พลังครอบครัว</th>
						<th>พลังสถานศึกษา</th>
				...
				<tbody>
					<tr parser-repeat="[data_list]" id="row_{record_number}">
						...
						<td>{self_management_score}</td>
						<td>{psychological_capital_score}</td>
						<td>{self_esteem_score}</td>
						<td>{identity_power_score}</td>
						<td>{compliance_score}</td>
						<td>{domestic_violence_score}</td>
						<td>{exemplary_score}</td>
						<td>{media_exposure_score}</td>
						<td>{attitude_score}</td>
						<td>{source_purchase_score}</td>
						<td>{family_power_score}</td>
						<td>{school_power_score}</td>
						<td>{friend_power_score}</td>
						<td>{community_power_score}</td>
						<td>{group_type_id}</td>
						<td>{data_year}</td>
						<td>{section}</td>
						<!-- <td>
