...
/**
 * Cms_ca_resuscitation_model Class
 * @date 2023-12-12
 */
class Cms_ca_resuscitation_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'cms_ca_resuscitation';
	...
	{
		$ca_resuscitation_id = checkEncryptData($data['ca_resuscitation_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.ca_resuscitation_id = $ca_resuscitation_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.ca_resuscitation_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				'ca_resuscitation_name' => $post['ca_resuscitation_name']
				,'ca_resuscitation_detail' => $post['ca_resuscitation_detail']
				,'image' => $post['image']
				,'img' => $post['img']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'ca_resuscitation_id'){
				if(!is_numeric($value)){
					$value = 0;
				}
				$value = $value + 0;
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$ca_resuscitation_id = checkEncryptData($post['encrypt_ca_resuscitation_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.ca_resuscitation_id = $ca_resuscitation_id");
