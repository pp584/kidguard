...
/**
 * Ci_log_history_model Class
 * @date 2022-05-08
 */
class Ci_log_history_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'tb_ci_log_history';
	...
	public function create($post)
	{
		...
		$data = array(
			'log_edit_user' => $post['log_edit_user'], 'log_edit_datetime' => setDateToStandard($post['log_edit_datetime']), 'log_edit_remark' => $post['log_edit_remark'], 'log_edit_table' => $post['log_edit_table'], 'log_edit_table_pk_name' => $post['log_edit_table_pk_name'], 'log_edit_table_pk_value' => $post['log_edit_table_pk_value'], 'log_edit_condition' => $post['log_edit_condition'], 'log_login_id' => $post['log_login_id']
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($this->order_field != '') {
			...
			$order_by = ($order_by != '' ? ', ' : '') . " $this->my_table.log_id DESC";
		}

		if ($search_field != '' && $value != '') {
			$search_method_field = "$this->my_table.$search_field";
			$search_method_value = '';
			if ($search_field == 'log_id') {
				if (!is_numeric($value)) {
					$value = 0;
				}
				$value = $value + 0;
