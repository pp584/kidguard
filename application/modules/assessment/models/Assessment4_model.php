 ...
 * @date 2023-07-23
 */
class Assessment4_model extends MY_Model
{
	...
	public function update_code($assessment_code_running)
	{
		$data = array();
		$data['assessment_code'] =  $assessment_code_running;
			$id = $this->session->userdata('assessment_id');
		
		if (!empty($data)) {
			$this->set_table_name("assessment_form");
			$this->set_where("assessment_form.id = $id");
			return $this->update_record($data);
		} else {
			$this->error_message = 'ไม่พบข้อมูลที่เปลี่ยนแปลง';
		}
