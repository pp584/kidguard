...
/**
 * [ Controller File name : assessment_create.php ]
 */
class Assessment_create extends CRUD_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('assessment/assessment_create_model', 'assessment_create');
		$this->assessment_create->session_name = 'assessment_assessment_create';
		$this->data['page_url'] = site_url('assessment/assessment_create');

		$this->data['page_title'] = 'ONCB';

		$js_url = 'assets/js_modules/assessment/assessment_create.js?ft=' . filemtime('assets/js_modules/assessment/assessment_create.js');
	...
	protected function render_view($path)
	{
		...
		$this->parser->parse('template/front-end/assessmentcreate_view', $this->data);
	}
	...
	{
		$this->session->unset_userdata($this->assessment_create->session_name . '_search_field');
		$this->session->unset_userdata($this->assessment_create->session_name . '_value');
	...
	public function search()
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'assessment_create', 'class' => 'active', 'url' => '#'),
		);
		if (isset($_POST['submit'])) {
			$search_field =  $this->input->post('search_field', TRUE);
			$value = $this->input->post('txtSearch', TRUE);
			$arr = array($this->assessment_create->session_name . '_search_field' => $search_field, $this->assessment_create->session_name . '_value' => $value);
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->assessment_create->session_name . '_search_field');
			$value = $this->session->userdata($this->assessment_create->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->assessment_create->order_field = $field;
			$this->assessment_create->order_sort = $sort;
		}
		$results = $this->assessment_create->read($start_row, $per_page);
		$total_row = $results['total_row'];
		$search_row = $results['search_row'];
		$list_data = $this->setDataListFormat($results['list_data'], $start_row);


		$page_url = site_url('assessment/assessment_create');
		...
		$this->render_view('assessment/assessment_create/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'assessment_create', 'url' => site_url('assessment/assessment_create')),
		...
		if ($id == "") {
			...
			$results = $this->assessment_create->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('assessment/assessment_create/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		// $this->data['provinces_province_id_option_list'] = $this->assessment_create->returnOptionList("provinces", "id", "name_th");
		$this->render_view('assessment/assessment_create/add_view');
	}
	...
	public function formValidate()
	{
		...
		// $frm->set_rules('province_id', 'อาชีพของมารดา', 'trim|required|is_natural');
		// $frm->set_rules('province_id', 'จังหวัด', 'trim|required|is_natural');
		...
		// $frm->set_rules('question_7', '6.สถานภาพทางครอบครัว', 'trim|required|is_natural');
		// $frm->set_rules('question_8', '7.อาชีพของบิดา', 'trim|required|is_natural');

		// $frm->set_rules('question_12', '11.ท่านเคยถูกให้ลองเสพสารเสพติดหรือไม่[1=เคย,2=ไม่เคย]', 'trim|required|is_natural');
		// $frm->set_rules('question_13', '13. เมื่อท่านมีปัญหาท่านปรึกษาใครบ้าง (ตอบได้มากกว่า 1 ข้อ)', 'trim|callback_multiple_select[question_13]');
	...
	public function save()
	{
		...
		if ($message != '') {
			...
			$id = $this->assessment_create->create($post);
			if ($id != '') {
				$success = TRUE;
				...
				// add raw data 
				$this->assessment_create->create_rawdata("assessment1", $id);
				$this->assessment_create->create_rawdata("assessment2", $id);
				$this->assessment_create->create_rawdata("assessment3", $id);
				$this->assessment_create->create_rawdata("assessment4", $id);

				$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>';
			} else {
				$success = FALSE;
				$message = 'Error : ' . $this->assessment_create->error_message;
			}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				// $this->data['provinces_province_id_option_list'] = $this->assessment_create->returnOptionList("provinces", "id", "name_th");
				$this->render_view('assessment/assessment_create/edit_view');
			}
		}
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			$result = $this->assessment_create->update($post);
			if ($result == false) {
				$message = $this->assessment_create->error_message;
				$ok = FALSE;
			} else {
				$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->assessment_create->error_message;
				$ok = TRUE;
			}
			$json = json_encode(array(
				'is_successful' => $ok,
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->assessment_create->delete($post);
			if ($result == false) {
				$message = $this->assessment_create->error_message;
				$ok = FALSE;
			} else {
