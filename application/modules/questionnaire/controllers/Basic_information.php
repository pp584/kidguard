 ...
 */
class Basic_information extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('questionnaire/Basic_information_model', 'Basic_information');
		$this->Basic_information->session_name = 'questionnaire_basic_information';

		$this->data['page_url'] = site_url('questionnaire/basic_information');
		...
		$js_url = 'assets/js_modules/questionnaire/basic_information.js?ft='. filemtime('assets/js_modules/questionnaire/basic_information.js');
	...
	public function search()
	{
		...
		$page_url = site_url('questionnaire/basic_information');
		...
		$this->render_view('questionnaire/basic_information/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'Basic_information', 'url' => site_url('questionnaire/basic_information')),
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('questionnaire/basic_information/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		$this->render_view('questionnaire/basic_information/add_view');
	}
	...
	public function import_excel_form() 
	{
		...
		$this->render_view('questionnaire/basic_information/addnew_import_form');
	...
	{
		// load excel library
		$this->load->library('questionnaire/Excel');
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				...
				$this->data['provinces_province_option_list'] = $this->Basic_information->returnOptionList("provinces", "id", "name_th" );
				$this->render_view('questionnaire/basic_information/edit_view');
			}
		}
	}
	...
	public function export_excel() 
	{
		...
		$results = $this->Basic_information->read();
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('B1', 'ส่วน');
		$objPHPExcel->getActiveSheet()->SetCellValue('C1', 'เพศ');
		$objPHPExcel->getActiveSheet()->SetCellValue('D1', 'อายุ');
		$objPHPExcel->getActiveSheet()->SetCellValue('E1', 'สถานภาพการศึกษา');
		$objPHPExcel->getActiveSheet()->SetCellValue('F1', 'ระดับการศึกษาปัจจุบัน ชั้นมัธยมศึกษาปีที่');
		$objPHPExcel->getActiveSheet()->SetCellValue('G1', 'จำนวนพี่น้อง');
		$objPHPExcel->getActiveSheet()->SetCellValue('H1', 'อื่นๆ');
		$objPHPExcel->getActiveSheet()->SetCellValue('I1', 'สถานภาพทางครอบครัว');
		$objPHPExcel->getActiveSheet()->SetCellValue('J1', 'อาชีพของบิดา');
		$objPHPExcel->getActiveSheet()->SetCellValue('K1', 'อื่นๆ');
		$objPHPExcel->getActiveSheet()->SetCellValue('L1', 'อาชีพของมารดา');
		$objPHPExcel->getActiveSheet()->SetCellValue('M1', 'อื่นๆ');
		$objPHPExcel->getActiveSheet()->SetCellValue('N1', 'รายได้ของครบอครัว');
		$objPHPExcel->getActiveSheet()->SetCellValue('O1', 'เมื่อท่านมีปัญหาท่านปรึกษาใครบ้าง(ตอบได้มากกว่า1ข้อ)');
		$objPHPExcel->getActiveSheet()->SetCellValue('P1', 'อื่นๆ');
		$objPHPExcel->getActiveSheet()->SetCellValue('Q1', 'ท่านเคยถูกให้ลองสารเสพติดหรือไม่');
		$objPHPExcel->getActiveSheet()->SetCellValue('R1', 'ท่านเคยลองใช้สารเสพติดใดบ้าง(ตอบได้มากกว่า1ข้อ)');
		$objPHPExcel->getActiveSheet()->SetCellValue('S1', 'อื่นๆ');
		...
		foreach ($data_lists as $row) {
			...
			$sheet->SetCellValue('A' . $rowCount, $row['provinceNameTh']);
			$sheet->SetCellValue('B' . $rowCount, $row['section']);
			$sheet->SetCellValue('C' . $rowCount, $row['preview_sex']);
			$sheet->setCellValueExplicit('D' . $rowCount, $row['age'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('E' . $rowCount, $row['preview_edu1'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('F' . $rowCount, $row['preview_edu_id'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('G' . $rowCount, $row['preview_sisbro'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('H' . $rowCount, $row['sisbro_remain'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('I' . $rowCount, $row['preview_family_status'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('J' . $rowCount, $row['preview_dad_job'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('K' . $rowCount, $row['dad_job_remain'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('L' . $rowCount, $row['preview_mom_job'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('M' . $rowCount, $row['mom_job_remain'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('N' . $rowCount, $row['preview_family_earn'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('O' . $rowCount, $row['preview_consult'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('P' . $rowCount, $row['consult_remain'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('Q' . $rowCount, $row['preview_try_drugs'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('R' . $rowCount, $row['preview_type_drugs'], PHPExcel_Cell_DataType::TYPE_STRING);
			$sheet->setCellValueExplicit('S' . $rowCount, $row['type_drugs_remain'], PHPExcel_Cell_DataType::TYPE_STRING);
		...
		$filename = "Basic_information_list". date("Y-m-d-H-i-s").".xlsx";
