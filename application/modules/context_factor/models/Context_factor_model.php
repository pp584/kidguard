...
/**
 * Context_factor_model Class
 * @date 2023-07-18
 */
class Context_factor_model extends MY_Model
{
	...
	{
		$context_factor_id = checkEncryptData($data['context_factor_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.context_factor_id = $context_factor_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.context_factor_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				'basic_information_id' => $post['basic_information_id']
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'context_factor_id'){
				if(!is_numeric($value)){
					$value = 0;
				}
				$value = $value + 0;
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$context_factor_id = checkEncryptData($post['encrypt_context_factor_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.context_factor_id = $context_factor_id");
