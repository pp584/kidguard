...
<div class="card">
	<div class="card-header bg-primary">
		<h3 class="card-title"><i class="fa fa-list-alt"></i> นำเข้า/นำออกไฟล์ ไฟล์ข้อมูลผู้ป่วย</h3>
	</div>
	<div class="card-body">
		<div class="row">
				...
				<!-- <form class="form-inline well well-sm" name="formSearch" method="post" action="{page_url}/search">
					...
					<div class="form-group">
						<select class="form-control" name="search_field" class="span2">
							<option value="patient_profile_id">ID</option>
						</select>
					</div>
					<div class="form-group">
				...
				<div class="pull-right text-right">
					<div class="form-group">
						<!-- <select class="form-control" id="set_order_by" class="span2" value="{order_by}">
							...
							<option value="patient_profile_id|asc">ID น้อย - มาก</option>
							<option value="patient_profile_id|desc">ID มาก - น้อย</option>
						</select> -->
					</div>
				</div>
			</div>
			...
			<table class="table table-bordered table-striped table-hover">
				...
				<tbody>
					<tr parser-repeat="[data_list]" id="row_{record_number}">
						...
						<td>{serviceInformationIdServiceUnitName}</td>
						<td>{ohac_id}</td>
						<td>{patient_name}</td>
						<td>{age}</td>
						<td>{preview_sex}</td>
