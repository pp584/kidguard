...
/**
 * Service_unit_name_model Class
 * @date 2022-04-01
 */
class Service_unit_name_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'service_unit_name';
	...
	{
		$service_unit_name = checkEncryptData($data['service_unit_name']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.service_unit_name = '$service_unit_name'");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.service_unit_name = '$id'");
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'service_unit_name'){
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$service_unit_name = checkEncryptData($post['encrypt_service_unit_name']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.service_unit_name = '$service_unit_name'");
