...
/**
 * Basic_information_model Class
 * @date 2023-06-01
 */
class Basic_information_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'basic_information';
	...
	{
		$basic_information_id = checkEncryptData($data['basic_information_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.basic_information_id = $basic_information_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.basic_information_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				...
				,'edu_id' => $post['edu_id']
				,'sisbro' => $post['sisbro']
				,'sisbro_remain' => $post['sisbro_remain']
				,'family_status' => $post['family_status']
				,'dad_job' => $post['dad_job']
				,'dad_job_remain' => $post['dad_job_remain']
				,'mom_job' => $post['mom_job']
				,'mom_job_remain' => $post['mom_job_remain']
				,'family_earn' => $post['family_earn']
				,'consult' => $post['consult']
				,'consult_remain' => $post['consult_remain']
				,'try_drugs' => $post['try_drugs']
				,'type_drugs' => $post['type_drugs']
				,'type_drugs_remain' => $post['type_drugs_remain']
				,'basic_infomation_id' => $_SESSION['basic_infomation_id']
		);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			if($search_field == 'province'){
				$search_method_field = "provinces_1.name_th";
		...
		if ($where != '') {
			$this->db->join('provinces AS provinces_1', "$this->my_table.province = provinces_1.id", 'left');
		...
		$this->db->select("$this->my_table.*, provinces_1.name_th AS provinceNameTh
				");
		$this->db->join('provinces AS provinces_1', "$this->my_table.province = provinces_1.id", 'left');
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$basic_information_id = checkEncryptData($post['encrypt_basic_information_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.basic_information_id = $basic_information_id");
