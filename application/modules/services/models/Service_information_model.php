...
/**
 * Service_information_model Class
 * @date 2022-04-06
 */
class Service_information_model extends MY_Model
{
	...
	public function create($post)
	{
		...
		$data = array(
			'master_ohca_id' => $post['service_unit_name'],'service_unit_name' => $post['service_unit_name'], 'operating_command' => $post['operating_command'], 'zone_area' => $post['zone_area'], 'police_station' => $post['police_station'], 'operating_number' => $post['operating_number'], 'regis_date' => setDateToStandard($post['regis_date']), 'performance' => $post['performance'], 'locale' => $post['locale'], 'event' => $post['event'], 'create_by_userid' => $post['create_by_userid'], 'create_date' => date('Y-m-d H:i:s')
		);
		$this->set_table_name($this->my_table);
	...
	public function create_time_distance($post)
	{
		$data = array(
	...
	public function create_event_information($post)
	{
		$data = array(
			'service_information_id' => $post['service_information_id']
		);
		$this->set_table_name("event_information");
		return $this->add_record($data);
	}

	public function create_basic_resuscitation($post)
	{
		$data = array(
			'service_information_id' => $post['service_information_id']
		);
		$this->set_table_name("basic_resuscitation");
		return $this->add_record($data);
	}

	public function create_treatment_information($post)
	{
		$data = array(
			'service_information_id' => $post['service_information_id']
		);
		$this->set_table_name("treatment_information");
		return $this->add_record($data);
	}

	public function create_delivery_information($post)
	{
		$data = array(
			'service_information_id' => $post['service_information_id']
		);
		$this->set_table_name("delivery_information");
		return $this->add_record($data);
	}

	public function create_result_data($post)
	{
		$data = array(
			'service_information_id' => $post['service_information_id']
		);
		$this->set_table_name("result_data");
		return $this->add_record($data);
	}
	...
	{
		$this->set_table_name('service_information_users');
		$this->db->select("service_information_users.*, tb_members_1.firstname AS detailUserIdFirstname, tb_members_1.lastname AS detailUserIdLastname");
		$this->db->join('tb_members AS tb_members_1', "service_information_users.user_id = tb_members_1.userid", 'left');
	...
	public function save_detail_list($post)
	{
		$this->set_table_name('service_information_users');
		$data = array(
			'user_id' => $post['user_id'], 'service_information_id' => $post['service_information_id']
		);
