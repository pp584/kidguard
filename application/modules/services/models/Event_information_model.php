...
/**
 * Event_information_model Class
 * @date 2022-04-23
 */
class Event_information_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'event_information';
	...
	{
		$event_information_id = checkEncryptData($data['event_information_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.event_information_id = $event_information_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.event_information_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				...
				,'even_scene' => $post['even_scene']
				,'even_scene_remark' => $post['even_scene_remark']
				,'accommodation' => $post['accommodation']
				,'accommodation_remark' => $post['accommodation_remark']
				,'workplace' => $post['workplace']
				,'workplace_remark' => $post['workplace_remark']
				,'public_place' => $post['public_place']
				,'eyewitnesses' => $post['eyewitnesses']
				,'general_public' => $post['general_public']
				,'medical_personnel' => $post['medical_personnel']
				,'eyewitnesses_remark' => $post['eyewitnesses_remark']
				,'causes_cardiac_arres' => $post['causes_cardiac_arres']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'event_information_id'){
				if(!is_numeric($value)){
					$value = 0;
				}
				$value = $value + 0;
	...
	public function update($post)
	{
		$data = array(
				'service_information_id' => $post['service_information_id']
				,'even_scene' => isset($post['even_scene'])
				,'even_scene_remark' => isset($post['even_scene_remark'])
				,'accommodation' => isset($post['accommodation'])
				,'accommodation_remark' => isset($post['accommodation_remark'])
				,'workplace' => isset($post['workplace'])
				,'workplace_remark' => $post['workplace_remark']
				,'public_place' => isset($post['public_place'])
				,'eyewitnesses' => isset($post['eyewitnesses'])
				,'general_public' => isset($post['general_public'])
				,'medical_personnel' => isset($post['medical_personnel'])
				,'eyewitnesses_remark' => isset($post['eyewitnesses_remark'])
				,'causes_cardiac_arres' => isset($post['causes_cardiac_arres'])
		);

		if(!empty($data)){
			$event_information_id = checkEncryptData($post['encrypt_event_information_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.event_information_id = $event_information_id");
