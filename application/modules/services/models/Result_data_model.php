...
/**
 * Result_data_model Class
 * @date 2022-04-23
 */
class Result_data_model extends MY_Model
{
	...
	{
		$result_data_id = checkEncryptData($data['result_data_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.result_data_id = $result_data_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.result_data_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
			'service_information_id' => $post['service_information_id'], 'patient_status' => $post['patient_status'], 'patient_status_dead' => $post['patient_status_dead'], 'department' => $post['department'], 'survival' => $post['survival'], 'days_admitted' => $post['days_admitted'], 'date_out' => setDateToStandard($post['date_out']), 'date_out_course' => $post['date_out_course']
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($search_field != '' && $value != '') {
			...
			$search_method_value = '';
			if ($search_field == 'result_data_id') {
				if (!is_numeric($value)) {
					$value = 0;
				}
				$value = $value + 0;
	...
	public function update($post)
	{
		$data = array(
			...
			, 'patient_status' => isset($post['patient_status'])
			, 'patient_status_dead' => isset($post['patient_status_dead'])
			, 'department' => isset($post['department'])
			, 'survival' => isset($post['survival'])
			, 'days_admitted' => isset($post['days_admitted'])
			, 'date_out' => setDateToStandard($post['date_out'])
			, 'date_out_course' => isset($post['date_out_course'])
		);

		if (!empty($data)) {
			$result_data_id = checkEncryptData($post['encrypt_result_data_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.result_data_id = $result_data_id");
