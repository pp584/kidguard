...
/**
 * Service_patient_profile_model Class
 * @date 2022-04-06
 */
class Service_patient_profile_model extends MY_Model
{
	...
	*/
	...
	{
		$this->set_table_name('patient_profile');
	...
	{
		$this->set_table_name('patient_profile');
		$this->set_where("patient_profile_id = $id");
	...
	public function save_detail_list($post)
	{
		...
		if($post['encrypt_patient_profile_id'] != ''){
			$patient_profile_id = checkEncryptData($post['encrypt_patient_profile_id']);
			$this->set_where("patient_profile_id = $patient_profile_id");
			return $this->update_record($data);
		}else{
			return $this->add_record($data);
		}
