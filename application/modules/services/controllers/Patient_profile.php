...
class Patient_profile extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('services/Patient_profile_model', 'Patient_profile');
		$this->Patient_profile->session_name = 'services_patient_profile';
		$this->load->model('Globalmodel', 'modeldb');

		$this->data['page_url'] = site_url('services/patient_profile');
		...
		$js_url = 'assets/js_modules/services/patient_profile.js?ft=' . filemtime('assets/js_modules/services/patient_profile.js');
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'ข้อมูลผู้ป่วย', 'class' => 'active', 'url' => '#'),
		);
		...
		$page_url = site_url('services/patient_profile');
		...
		$this->render_view('services/patient_profile/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'Patient_profile', 'url' => site_url('services/patient_profile')),
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('services/patient_profile/preview_view');
			}
		}
	}
	...
	public function setFormAddData()
	{
		...
		$operating_number = $this->table('service_information')->get_value('operating_number')->where("service_information_id = '$service_information_id'");

		// $master_ohca_id = $this->table('services')->get_value('code_name')->where("services_name = '$service_unit_name'");

		$number = $this->get_autonumber();

		$this->data['master_ohca_id'] = $operating_number.$number;
	...
	public function get_autonumber()
	{
		...
		$data_cms_ca_resuscitation = $this->modeldb->get_list("patient_profile","service_information_id=$service_information_id");
		return count($data_cms_ca_resuscitation)+1;
	}
	...
	public function add()
	{
		...
		$this->setFormAddData();
		$this->render_view('services/patient_profile/add_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'ข้อมูลผู้ป่วย', 'url' => site_url('services/patient_profile')),
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('services/patient_profile/edit_view');
			}
		}
	}
