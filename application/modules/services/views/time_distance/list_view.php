...
<div class="card">
	<div class="card-header bg-primary">
		<h3 class="card-title"><i class="fa fa-list-alt"></i> ตารางแสดงรายการ ข้อมูล<b>time_distance</b></h3>
	</div>
	<div class="card-body">
		<div class="row">
			...
			<div class="col-sm-12 col-md-9">
				<form class="form-inline well well-sm" name="formSearch" method="post" action="{page_url}/search">
					...
					<option value="itme_distance_id">ID</option>
						</select>
 					</div>
					<div class="form-group">
			...
			<div class="col-sm-12 col-md-3">
				<div class="pull-right text-right">
					...
					<option value="itme_distance_id|asc">ID น้อย - มาก</option><option value="itme_distance_id|desc">ID มาก - น้อย</option>
						</select>
 					</div>
				</div>
			</div>
			...
			<table class="table table-bordered table-striped table-hover">
				...
				<tbody>
					<tr parser-repeat="[data_list]" id="row_{record_number}">
						...
						<td>{incident_time}</td>
						<td>{incident_time_recrive}</td>
						<td>{time_order}</td>
						<td>{base_time_start}</td>
						<td>{base_time_stop}</td>
						<td>{time_of_leaving}</td>
						<td>{time_to_hospital}</td>
						<td>{base_time_finish}</td>
						<td>{distance_base_scene}</td>
							...
							<div class="btn-group pull-right">
								...
								<a href="javascript:void(0);" class="btn-delete-row my-tooltip btn btn-danger btn-sm"
									data-toggle="tooltip" title="ลบรายการนี้"
									 data-itme_distance_id = "{encrypt_itme_distance_id}" data-row-number="{record_number}">
...
<label class="col-sm-3 text-right badge badge-warning" for="edit_remark">ระบุเหตุผล :</label>
					...
					<input type="hidden" name="encrypt_itme_distance_id" />
