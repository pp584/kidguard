...
<div class="card">
	<div class="card-header bg-primary">
		<h3 class="card-title"><i class="fa fa-list-alt"></i> สืบค้นและปรับปรุงข้อมูลผู้ป่วย</h3>
	</div>
	<div class="card-body">
		<div class="row">
			<div class="col-sm-12 col-md-12 mb-3">
				<div class="text-right">
					<a href="{page_url}/add" class="btn btn-success btn-lg" data-toggle="tooltip" title="เพิ่มข้อมูลใหม่">
						<i class="fa fa-plus-square"></i></span> เพิ่มข้อมูลผู้ป่วยใหม่
					</a>
				</div>
			</div>
				...
				<form class="form-inline well well-sm" name="formSearch" method="post" action="{page_url}/search">
					...
					<div class="form-group">
						<select class="form-control" name="search_field" class="span2">
							<option value="service_information_id">ID</option>
						</select>
					</div>
					<div class="form-group">
				...
				<div class="pull-right text-right">
					<div class="form-group">
						<select class="form-control" id="set_order_by" class="span2" value="{order_by}">
							...
							<option value="service_information_id|asc">ID น้อย - มาก</option>
							<option value="service_information_id|desc">ID มาก - น้อย</option>
						</select>
					</div>
				</div>
			</div>
			...
			<table class="table table-bordered table-striped table-hover">
				...
				<tbody>
					<tr parser-repeat="[data_list]" id="row_{record_number}">
						...
						<td>{service_information_id}</td>
						<td>{serviceUnitNameServicesName}</td>
						<td>{operating_command}</td>
						<td>{zone_area}</td>
						...
						<td style="text-align:center;">
							<div class="btn-group">
								...
								<a href="javascript:void(0);" class="btn-delete-row my-tooltip btn btn-danger btn-sm" data-toggle="tooltip" title="ลบรายการนี้" data-service_information_id="{encrypt_service_information_id}" data-row-number="{record_number}">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<form id="formDelete">
					...
					<input type="hidden" name="encrypt_service_information_id" />
