...
/**
 * [ Controller File name : Cms_about_us.php ]
 */
class Cms_about_us extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('website/Cms_about_us_model', 'Cms_about_us');
		$this->Cms_about_us->session_name = 'website_cms_about_us';

		$this->load->model('FileUpload_model', 'FileUpload');
		$this->data['page_url'] = site_url('website/cms_about_us');
		...
		$this->upload_store_path = './assets/uploads/cms_about_us/';
		...
		$js_url = 'assets/js_modules/website/cms_about_us.js?ft='. filemtime('assets/js_modules/website/cms_about_us.js');
	...
	public function index()
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'Cms_about_us', 'class' => 'active', 'url' => '#'),
		);
	...
	public function list_all() {
		$this->session->unset_userdata($this->Cms_about_us->session_name . '_search_field');
		$this->session->unset_userdata($this->Cms_about_us->session_name . '_value');
	...
	public function search()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$arr = array($this->Cms_about_us->session_name . '_search_field' => $search_field, $this->Cms_about_us->session_name . '_value' => $value );
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->Cms_about_us->session_name . '_search_field');
			$value = $this->session->userdata($this->Cms_about_us->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->Cms_about_us->order_field = $field;
			$this->Cms_about_us->order_sort = $sort;
		}
		$results = $this->Cms_about_us->read($start_row, $per_page);
		$total_row = $results['total_row'];
		$search_row = $results['search_row'];
		$list_data = $this->setDataListFormat($results['list_data'], $start_row);


		$page_url = site_url('website/cms_about_us');
		...
		$this->render_view('website/cms_about_us/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'Cms_about_us', 'url' => site_url('website/cms_about_us')),
		...
		if ($id == "") {
			...
			$results = $this->Cms_about_us->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('website/cms_about_us/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		$this->render_view('website/cms_about_us/add_view');
	}
	...
	public function formValidate()
	{
		...
		$frm->set_rules('title_th', 'title_th', 'trim|required');
		$frm->set_rules('title_en', 'title_en', 'trim|required');
		$frm->set_rules('detail', 'detail', 'trim|required');
		$frm->set_rules('cardiac_arrest', 'cardiac_arrest', 'trim|required');
		...
		if ($frm->run() == FALSE) {
			...
			$message .= form_error('title_th');
			$message .= form_error('title_en');
			$message .= form_error('detail');
			$message .= form_error('cardiac_arrest');
			return $message;
		}
	}
	...
	public function save()
	{
		...
		if ($message != '') {
			...
			if($upload_error == 0){
				$success = TRUE;
				$id = $this->Cms_about_us->create($post);
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'จัดการข้อมูลเกี่ยวกับเรา', 'url' => '#'),
		...
		if ($id == "") {
			...
			$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				...
				$this->render_view('website/cms_about_us/edit_view');
			}
		}
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			if($upload_error == 0){
				$result = $this->Cms_about_us->update($post);
				if($result == false){
					$message = $this->Cms_about_us->error_message;
					$ok = FALSE;
				}else{
					$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Cms_about_us->error_message;
					$ok = TRUE;
				}
			}else{
				$ok = FALSE;
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->Cms_about_us->delete($post);
			if($result == false){
				$message = $this->Cms_about_us->error_message;
				$ok = FALSE;
			}else{
	...
	private function setPreviewFormat($row_data)
	{
		...
		$this->data['record_title_th'] = $data['title_th'];
		$this->data['record_title_en'] = $data['title_en'];
		$this->data['record_detail'] = $data['detail'];
		$this->data['record_cardiac_arrest'] = $data['cardiac_arrest'];
