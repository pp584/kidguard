...
/**
 * [ Controller File name : Research.php ]
 */
class Research extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('website/Research_model', 'Research');
		$this->Research->session_name = 'website_research';

		$this->load->model('FileUpload_model', 'FileUpload');
		$this->data['page_url'] = site_url('website/research');
		...
		$this->upload_store_path = './assets/uploads/research/';
		...
		$js_url = 'assets/js_modules/website/research.js?ft='. filemtime('assets/js_modules/website/research.js');
	...
	public function index()
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'จัดการข้อมูลงานวิจัย', 'class' => 'active', 'url' => '#'),
		);
	...
	public function list_all() {
		$this->session->unset_userdata($this->Research->session_name . '_search_field');
		$this->session->unset_userdata($this->Research->session_name . '_value');
	...
	public function search()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$arr = array($this->Research->session_name . '_search_field' => $search_field, $this->Research->session_name . '_value' => $value );
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->Research->session_name . '_search_field');
			$value = $this->session->userdata($this->Research->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->Research->order_field = $field;
			$this->Research->order_sort = $sort;
		}
		$results = $this->Research->read($start_row, $per_page);
		$total_row = $results['total_row'];
		$search_row = $results['search_row'];
		$list_data = $this->setDataListFormat($results['list_data'], $start_row);


		$page_url = site_url('website/research');
		...
		$this->render_view('website/research/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'จัดการข้อมูลงานวิจัย', 'url' => site_url('website/research')),
		...
		if ($id == "") {
			...
			$results = $this->Research->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('website/research/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		$this->data['tb_members_create_by_userid_option_list'] = $this->Research->returnOptionList("tb_members", "userid", "CONCAT_WS(' - ', firstname,lastname)" );
		...
		$this->setFormAddData();
		$this->render_view('website/research/add_view');
	}
	...
	public function formValidate()
	{
		...
		$frm->set_rules('research_name', 'ผู้วิจัย', 'trim|required');
		//file upload
		$check_file = FALSE;
		if($this->input->post('files_label') == ''){
			$check_file = TRUE;
		}
		if($check_file == TRUE){
			if(empty($_FILES['files']['name'])){
				$frm->set_rules('files', 'files', 'trim|required');
			}
		}
		...
		if ($frm->run() == FALSE) {
			...
			$message .= form_error('research_name');
	...
	public function formValidateWithFile()
	{
		...
		if(!empty($_FILES['files']['name'])){
			...
			$frm->set_rules('files', 'files', 'callback_file_check');
			if ($frm->run() == FALSE) {
				$message .= form_error('files');
			}
		}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				...
				$this->data['tb_members_create_by_userid_option_list'] = $this->Research->returnOptionList("tb_members", "userid", "CONCAT_WS(' - ', firstname,lastname)" );
				$this->render_view('website/research/edit_view');
			}
		}
	}
	...
	{
		$error = '';
		$cms_researchs_id = ci_decrypt($data['encrypt_cms_researchs_id']);
		if($cms_researchs_id==''){
			$error .= '- รหัส cms_researchs_id';
		}
		return $error;
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			if($upload_error == 0){
				...
				$result = $this->Research->update($post);
				if($result == false){
					$message = $this->Research->error_message;
					$ok = FALSE;
				}else{
					$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Research->error_message;
					$ok = TRUE;
				}
			}else{
				$ok = FALSE;
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->Research->delete($post);
			if($result == false){
				$message = $this->Research->error_message;
				$ok = FALSE;
			}else{
	...
	private function setDataListFormat($lists_data, $start_row=0)
	{
		...
		for($i=0;$i<$count;$i++){
			...
			$pk1 = $data[$i]['cms_researchs_id'];
			$data[$i]['url_encrypt_id'] = urlencode(encrypt($pk1));

			if($pk1 != ''){
				$pk1 = ci_encrypt($pk1);
			}
			$data[$i]['encrypt_cms_researchs_id'] = $pk1;
	...
	private function setPreviewFormat($row_data)
	{
		...
		$pk1 = $data['cms_researchs_id'];
		$this->data['recode_url_encrypt_id'] = urlencode(encrypt($pk1));

		if($pk1 != ''){
			$pk1 = ci_encrypt($pk1);
		}
		$this->data['encrypt_cms_researchs_id'] = $pk1;
		...
		$this->data['record_cms_researchs_id'] = $data['cms_researchs_id'];
		...
		$this->data['record_research_name'] = $data['research_name'];
