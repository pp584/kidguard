...
/**
 * [ Controller File name : Slides.php ]
 */
class Slides extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('website/Slides_model', 'Slides');

		$this->Slides->session_name = 'website_slides';

		$this->load->model('FileUpload_model', 'FileUpload');
		$this->data['page_url'] = site_url('website/slides');
		...
		$this->upload_store_path = './assets/uploads/slides/';
		...
		$js_url = 'assets/js_modules/website/slides.js?ft=' . filemtime('assets/js_modules/website/slides.js');
	...
	public function index()
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'Slides', 'class' => 'active', 'url' => '#'),
		);
	...
	public function list_all()
	{
		...
		$this->session->unset_userdata($this->Slides->session_name . '_search_field');
		$this->session->unset_userdata($this->Slides->session_name . '_value');
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'จัดการ Slider', 'class' => 'active', 'url' => '#'),
		);

		if (isset($_POST['submit'])) {
			$search_field =  $this->input->post('search_field', TRUE);
			$value = $this->input->post('txtSearch', TRUE);
			$arr = array($this->Slides->session_name . '_search_field' => $search_field, $this->Slides->session_name . '_value' => $value);
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->Slides->session_name . '_search_field');
			$value = $this->session->userdata($this->Slides->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->Slides->order_field = $field;
			$this->Slides->order_sort = $sort;
		}

		$results = $this->Slides->read($start_row, $per_page);
		...
		$page_url = site_url('website/slides');
		...
		$this->render_view('website/slides/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'จัดการ Slider', 'url' => site_url('website/slides')),
		...
		if ($id == "") {
			...
			$results = $this->Slides->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('website/slides/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		$this->render_view('website/slides/add_view');
	}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('website/slides/edit_view');
			}
		}
	}
	...
	{
		$error = '';
		$cms_slide_id = ci_decrypt($data['encrypt_cms_slide_id']);
		if ($cms_slide_id == '') {
			$error .= '- รหัส cms_slide_id';
		}
		return $error;
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			if ($upload_error == 0) {
				$result = $this->Slides->update($post);
				if ($result == false) {
					$message = $this->Slides->error_message;
					$ok = FALSE;
				} else {
					$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Slides->error_message;
					$ok = TRUE;
				}
			} else {
				$ok = FALSE;
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->Slides->delete($post);
			if ($result == false) {
				$message = $this->Slides->error_message;
				$ok = FALSE;
			} else {
	...
	private function setDataListFormat($lists_data, $start_row = 0)
	{
		...
		for ($i = 0; $i < $count; $i++) {
			...
			$pk1 = $data[$i]['cms_slide_id'];
			$data[$i]['url_encrypt_id'] = urlencode(encrypt($pk1));

			if ($pk1 != '') {
				$pk1 = ci_encrypt($pk1);
			}
			$data[$i]['encrypt_cms_slide_id'] = $pk1;
	...
	private function setPreviewFormat($row_data)
	{
		...
		$pk1 = $data['cms_slide_id'];
		$this->data['recode_url_encrypt_id'] = urlencode(encrypt($pk1));

		if ($pk1 != '') {
			$pk1 = ci_encrypt($pk1);
		}
		$this->data['encrypt_cms_slide_id'] = $pk1;

		$this->data['record_cms_slide_id'] = $data['cms_slide_id'];
		...
		$this->data['record_sequences'] = $data['sequences'];
	}
}
