...
class Cms_ca_basic_info_model extends MY_Model
{
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_members AS tb_members_1', "$this->my_table.create_by_userid = tb_members_1.userid", 'left');
		...
		$this->db->select("$this->my_table.*, tb_members_1.firstname AS createByUseridFirstname, tb_members_1.lastname AS createByUseridLastname
				");
		$this->db->join('tb_members AS tb_members_1', "$this->my_table.create_by_userid = tb_members_1.userid", 'left');
