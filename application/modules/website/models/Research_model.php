...
class Research_model extends MY_Model
{
	...
	{
		$cms_researchs_id = checkEncryptData($data['cms_researchs_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.cms_researchs_id = $cms_researchs_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.cms_researchs_id = $id");
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'cms_researchs_id'){
				if(!is_numeric($value)){
					$value = 0;
				}
				$value = $value + 0;
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$cms_researchs_id = checkEncryptData($post['encrypt_cms_researchs_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.cms_researchs_id = $cms_researchs_id");
