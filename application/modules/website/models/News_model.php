...
class News_model extends MY_Model
{
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('cms_category AS cms_category_1', "$this->my_table.category_id = cms_category_1.id", 'left');
		$this->db->join('tb_members AS tb_members_2', "$this->my_table.userid = tb_members_2.userid", 'left');
		...
		$this->db->select("$this->my_table.*, cms_category_1.name AS categoryIdName
				, tb_members_2.firstname AS useridFirstname, tb_members_2.lastname AS useridLastname
				");
		$this->db->join('cms_category AS cms_category_1', "$this->my_table.category_id = cms_category_1.id", 'left');
		$this->db->join('tb_members AS tb_members_2', "$this->my_table.userid = tb_members_2.userid", 'left');
