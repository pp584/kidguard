...
class Contact_us_model extends MY_Model
{
	...
	{
		$cms_contact_us_id = checkEncryptData($data['cms_contact_us_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.cms_contact_us_id = $cms_contact_us_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.cms_contact_us_id = $id");
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'cms_contact_us_id'){
				if(!is_numeric($value)){
					$value = 0;
				}
				$value = $value + 0;
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$cms_contact_us_id = checkEncryptData($post['encrypt_cms_contact_us_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.cms_contact_us_id = $cms_contact_us_id");
