...
class Users_model extends MY_Model
{
	...
	{
		$data = array(
			'username' => $post['username'], 'email' => $post['email'], 'password' => pass_secure_hash($post['password']), 'prefix' => $post['prefix'], 'firstname' => $post['firstname'], 'lastname' => $post['lastname'], 'sex' => $post['sex'], 'level' => $post['level'], 'tel_number' => $post['tel_number'], 'line_id' => $post['line_id'], 'photo' => $post['photo'], 'province_id' => $post['province_id'], 'university_name' => $post['university_name']
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			...
			$this->db->join('services AS services_3', "$this->my_table.department_id = services_3.services_id", 'left');
		...
		$this->db->select("$this->my_table.*, tb_members_prefix_1.prefix_name AS prefixPrefixName
				...
				, services_3.services_name AS departmentIdServicesName
				");
		$this->db->join('tb_members_prefix AS tb_members_prefix_1', "$this->my_table.prefix = tb_members_prefix_1.prefix_name", 'left');
		$this->db->join('tb_members_level AS tb_members_level_2', "$this->my_table.level = tb_members_level_2.level_value", 'left');
		$this->db->join('services AS services_3', "$this->my_table.department_id = services_3.services_id", 'left');
	...
	public function update($post)
	{
		$data = array(
			...
			, 'prefix' => $post['prefix'], 'firstname' => $post['firstname'], 'lastname' => $post['lastname'], 'sex' => $post['sex'], 'level' => $post['level'], 'tel_number' => $post['tel_number'], 'line_id' => $post['line_id'], 'province_id' => $post['province_id'], 'university_name' => $post['university_name']
		);
