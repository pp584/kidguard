...
/**
 * Risk_behavior_model Class
 * @date 2023-07-18
 */
class Risk_behavior_model extends MY_Model
{
	...
	{
		$risk_behavior_id = checkEncryptData($data['risk_behavior_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.risk_behavior_id = $risk_behavior_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.risk_behavior_id = $id");
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'risk_behavior_id'){
				if(!is_numeric($value)){
					$value = 0;
				}
				$value = $value + 0;
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$risk_behavior_id = checkEncryptData($post['encrypt_risk_behavior_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.risk_behavior_id = $risk_behavior_id");
