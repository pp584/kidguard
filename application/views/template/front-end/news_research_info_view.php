			...
			<div class="container pb-5">
				<div class="row">
					<div class="col-12" class="text" style="text-shadow: 1px 3px 8px rgba(0, 0, 0, 1);">
						<span class="text text-white">Research</span>
						<h3 class="hero-text text-white my-2">งานวิจัย</h3>
						<h5 class="text-white"><a href="./" class="text text-white"><u>Home</u></a> - Research</h5>
					</div>
				</div>
			</div>
		</section>
								...
								<?php foreach ($results as $row) { ?>
									<article class="no-hover-shadow ms-news-detail-page">
										...
										<div class="ms-content-area bg-white">
											...
											<h4 class="font-weight-bold mt-3 ms-news-title">เรื่อง : <?php echo $row->subject ?></h4>
											<p class="mb-5">รายละเอียด : <br /> <?php echo $row->detail ?></p>
											<span class="ms-title-15">ผู้วิจัย</span>
											<p class="mb-5"><?php echo $row->research_name ?></p>
											<?php if ($row->files != '') { ?>
												<hr />
												<span class="ms-title-15">ไฟล์เพิ่มเติม</span>
												<p class="mb-5"><a href="{base_url}<?php echo $row->files ?>">Download ไฟล์เพิ่มเติม</a></p>
											<?php } ?>

											<span>วันที่เผยแพร่ <?php echo $row->create_date ?> </span> <span>ยอดเข้าชม <?= $row->page_views_total ?> ครั้ง</span>
										</div>
